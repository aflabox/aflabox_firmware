...
class ConfigManager:
    ...
    async def update_version(self,data):
        print(data)
        self.update_config("FIRMWARE_MANAGEMENT","next_version",data.get("version",None))
        self.update_config("FIRMWARE_MANAGEMENT","next_version_hash",data.get("hash",None))
    ...
    def _get_camera_presets(self):
        ...
        # Default preset
        if 'CAMERA_PRESET_DEFAULT' in self.config.sections():
            presets['default'] = {
    ...
    def get_queue_service_config(self):
        ...
        """
        if 'QUEUE_FILE_SERVICE' not in self.config.sections():
    ...
    def get_camera_system_config(self):
        ...
        """
        if 'CAMERA_SYSTEM' not in self.config.sections():
    ...
    def _get_available_resolutions(self):
        ...
        if 'AVAILABLE_RESOLUTIONS' in self.config.sections():
    ...
    def _get_available_thumbnails(self):
        ...
        if 'AVAILABLE_THUMBNAILS' in self.config.sections():
    ...
    def _set_default_config(self):
        ...
        self._set_default_config("UPLOADS_SETTINGS")
        self._set_default_config("ENDPOINTS")
        self._set_default_config("RABBITMQ_QUEUE")
        self._set_default_config("LED_PINS")
        self._set_default_config("DEFAULT_PINS")
        self._set_default_config("WEBSOCKET")
    ...
    def _set_section_defaults(self, section):
        ...
        # Queue File Service defaults
            ...
            defaults = {
                ...
                'ftp_user': 'qbox',
                'ftp_pass': 'Qb0x24',
                'ftp_remote_dir': '/ftp/uploads/raw',
                'use_tls': 'true',
                'verify_ssl': 'true',
                'max_retries': 3,
                'retry_delay': 5,
        ...
        # Available Thumbnails defaults
        ...
        elif section == 'UPLOADS_SETTINGS':
            defaults = {
                'upload_dir': '/home/aflabox/.images',
                'upload_url': 'http://aflabox.gror.app/upload',
                'upload_token': 'eyJhbGciOiJIUzI1NiIsInR5cCI',
                'upload_token_secret': '0b7f0e4b-0b7f-0e',
                'device_name': 'Aflabox-Unknown',
                'device_id': 0
            }
            for key, value in defaults.items():
                if key not in self.config[section]:
            ...
            defaults = {
                'base_url': 'https://api.aflabox.ai',
                'createtest': '/crop/tests'
            }
            for key, value in defaults.items():
                if key not in self.config[section]:
                    self.config[section][key] = value

        elif section == 'RABBITMQ_QUEUE':
            defaults = {
                'amq_url': 'amqp://admin:admin1234@95.110.228.29:5672'
            }
            for key, value in defaults.items():
                if key not in self.config[section]:
                    self.config[section][key] = value

        elif section == 'LED_PINS':
            defaults = {
                'mainbuttonled': 12,
                'powerled': 13
            }
            ...
            defaults = {
                'local_uri': 'ws://localhost:7896/ws',
                'local_channel': 'aflabox.local',
                'remote_uri': 'ws://95.110.228.29:7896/ws',
                'remote_channel': 'aflabox.main'
            }
            for key, value in defaults.items():
                if key not in self.config[section]:
                    self.config[section][key] = value
